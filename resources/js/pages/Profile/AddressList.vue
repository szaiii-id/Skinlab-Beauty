...
import { MapPin, Plus, Pencil, Trash2, Home, Briefcase, Star, ChevronDown, ChevronUp } from 'lucide-vue-next';
...
const showAllAddresses = ref(false); // State for Accordion
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 border-b border-gray-100 pb-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-lg font-bold text-gray-900">Address Book</h2>
                    <p class="text-xs text-gray-500">Manage your shipping addresses.</p>
                </div>
            </div>
            <button 
                @click="openCreateForm"
                class="flex items-center gap-2 bg-rose-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-rose-700 transition shadow-md hover:shadow-lg"
            >
                <Plus class="w-4 h-4" />
        ...
        <div v-else-if="addresses.length === 0" class="text-center py-10 border-2 border-dashed border-gray-100 rounded-xl bg-gray-50">
            ...
            <p class="text-gray-900 font-medium">No saved addresses</p>
        ...
        <div v-else class="space-y-4">
            ...
            <div v-if="defaultAddress" class="relative border rounded-xl p-5 transition-all hover:shadow-md border-rose-200 bg-rose-50/40">
                ...
                <div class="flex items-start gap-4">
                    <div class="mt-1 p-2 rounded-full bg-white text-rose-600 shadow-sm border border-rose-100">
                    ...
                    <div class="flex-1 pr-16">
                        ...
                        <h4 class="font-bold text-gray-900 text-base mb-1">{{ defaultAddress.receiver_name }}</h4>
                ...
                <div class="flex justify-end items-center gap-3 mt-4 pt-4 border-t border-rose-100">
            ...
            <div v-if="otherAddresses.length > 0">
                <button 
                    @click="toggleShowAll"
                    class="w-full flex items-center justify-between px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-lg border border-gray-200 text-sm font-medium text-gray-700 transition-colors mb-4"
                >
                ...
                <div v-if="showAllAddresses" class="space-y-4 animate-in slide-in-from-top-2 fade-in duration-300">
                    <div 
                        ...
                        :key="address.id"
                        class="relative border border-gray-200 bg-white rounded-xl p-5 transition-all hover:shadow-md hover:border-gray-300"
                    >
                        <div class="flex items-start gap-4">
                            <div class="mt-1 p-2 rounded-full bg-gray-50 text-gray-500">
                        ...
                        <div class="flex justify-end items-center gap-3 mt-4 pt-4 border-t border-gray-100">
                            <button 
                                @click="handleSetDefault(address.id)"
                                class="text-xs font-bold text-gray-400 hover:text-rose-600 mr-auto transition-colors uppercase tracking-wide"
                            >
                                Set as Default
                            </button>
